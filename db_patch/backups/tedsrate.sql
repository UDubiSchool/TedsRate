...
-- Procedures
--
CREATE DEFINER=`root`@`%.washington.edu` PROCEDURE `addArtifact`(IN `artifactName` VARCHAR(45), IN `artifactURL` VARCHAR(255), IN `artifactTypeID` INT, IN `artifactLanguageID` INT, OUT `newArtifactID` INT)
BEGIN
START TRANSACTION;
INSERT INTO artifact
(artifactName, artifactURL, artifactTypeID, artifactLanguageID)
VALUES 
(artifactName, artifactURL, artifactTypeID, artifactLanguageID);
COMMIT;
...
Start Transaction;
INSERT INTO persona
(personaName, personaDesc, personaLanguageID)
...
CREATE DEFINER=`root`@`%.washington.edu` PROCEDURE `addProject`(IN `projectName` VARCHAR(45), IN `projectDescription` VARCHAR(150), IN `projectLanguageID` INT, OUT `newProjectID` INT)
BEGIN
START TRANSACTION;
INSERT Into project
(projectName, projectDescription, projectLanguageID)
VALUES
(projectName, projectDescription, projectLanguageID);
COMMIT;
...
CREATE DEFINER=`root`@`%.washington.edu` PROCEDURE `addRating`(IN `ratingValue` DECIMAL, IN `categoryID` INT, IN `assessmentID` INT, OUT `newRatingID` INT)
IF (SELECT 1 = 1 FROM rating WHERE rating.assessmentID= assessmentID and rating.categoryID = categoryID) THEN
BEGIN
    UPDATE rating 
    SET ratingValue = ratingValue, 
    ratingID=LAST_INSERT_ID(ratingID)
    WHERE rating.assessmentID= assessmentID and rating.categoryID = categoryID;
    select LAST_INSERT_ID() into newRatingID;
END;
ELSE
BEGIN
    INSERT INTO rating
(ratingValue, categoryID, assessmentID)
VALUES (ratingValue, categoryID, assessmentID);
select LAST_INSERT_ID() into newRatingID;
END;
END IF$$

CREATE DEFINER=`root`@`%.washington.edu` PROCEDURE `addScenario`(IN `scenarioName` VARCHAR(45), IN `scenarioDescription` VARCHAR(255), IN `scenarioLanguageID` INT, OUT `newScenarioID` INT)
BEGIN
START TRANSACTION;
INSERT INTO scenario
(scenarioName, scenarioDescription, scenarioLanguageID)
VALUES
(scenarioName, scenarioDescription, scenarioLanguageID);
COMMIT;
...
SELECT a.artifactID as ID, artifact.artifactName as name, a.artifactURL as URL, atype.artifactTypeTitle as artType
from artifact a inner join artifactType atype on a.artifactTypeID = atype.artifactTypeID
WHERE a.artifactLanguageID = languageID;
END$$
...
FROM persona p
WHERE p.personaLanguageID = languageID;
END$$
...
CREATE DEFINER=`root`@`%.washington.edu` PROCEDURE `getCategories`(IN `criterionID` INT, OUT `categoryID` INT, IN `CategoryName` VARCHAR(45), OUT `categoryDesc` VARCHAR(255))
BEGIN

Select c.categoryName, c.categoryID, c.categoryDesc
From category c
...
From criterion
Where criterionLanguageID = languageID;
END$$
...
CREATE DEFINER=`root`@`%.washington.edu` PROCEDURE `updateCategory`(IN `categoryID` INT, IN `categoryName` VARCHAR(45), IN `categoryDesc` VARCHAR(255), IN `criterionID` INT, IN `categoryLanugageID` INT)
BEGIN
UPDATE Category
SET
...
categoryLanugageID = categoryLanguageID
...
CREATE TABLE IF NOT EXISTS `artifact` (
  ...
  `artifactLanguageID` int(11) NOT NULL DEFAULT '5',
...
INSERT INTO `artifact` (`artifactID`, `artifactName`, `artifactURL`, `artifactTypeID`, `artifactLanguageID`, `artifactDescription`) VALUES
...
INSERT INTO `assessment` (`assessmentID`, `assessmentIDHashed`, `userID`, `personaID`, `scenarioID`, `projectArtifactID`, `isComplete`, `completionDate`, `ratingUrl`) VALUES
(42, '73475cb40a568e8da8a045ced110137e159f890ac4da883b6b17dc651b3a8049', 42, 39, 36, 65, 'true', '2015-04-27 11:21:07', NULL),
(43, '44cb730c420480a0477b505ae68af508fb90f96cf0ec54c6ad16949dd427f13a', 32, 39, 36, 65, NULL, NULL, NULL),
(44, '71ee45a3c0db9a9865f7313dd3372cf60dca6479d46261f3542eb9346e4a04d6', 44, 39, 36, 65, NULL, NULL, NULL),
(45, '811786ad1ae74adfdd20dd0372abaaebc6246e343aebd01da0bfc4c02bf0106c', 45, 39, 36, 65, NULL, NULL, NULL),
(46, '25fc0e7096fc653718202dc30b0c580b8ab87eac11a700cba03a7c021bc35b0c', 47, 39, 36, 65, 'true', '2015-04-26 11:50:36', NULL),
(47, '31489056e0916d59fe3add79e63f095af3ffb81604691f21cad442a85c7be617', 48, 39, 36, 65, NULL, NULL, NULL),
(48, '98010bd9270f9b100b6214a21754fd33bdc8d41b2bc9f9dd16ff54d3c34ffd71', 49, 39, 36, 65, 'true', '2015-04-26 11:58:26', NULL),
(49, '0e17daca5f3e175f448bacace3bc0da47d0655a74c8dd0dc497a3afbdad95f1f', 50, 39, 36, 65, NULL, NULL, NULL),
(50, '1a6562590ef19d1045d06c4055742d38288e9e6dcd71ccde5cee80f1d5a774eb', 32, 39, 37, 65, 'true', '2015-04-23 16:04:13', NULL),
(52, '41cfc0d1f2d127b04555b7246d84019b4d27710a3f3aff6e7764375b1e06e05d', 44, 39, 37, 65, NULL, NULL, NULL),
(53, '2858dcd1057d3eae7f7d5f782167e24b61153c01551450a628cee722509f6529', 45, 39, 37, 65, NULL, NULL, NULL),
(54, '2fca346db656187102ce806ac732e06a62df0dbb2829e511a770556d398e1a6e', 47, 39, 37, 65, NULL, NULL, NULL),
(55, '02d20bbd7e394ad5999a4cebabac9619732c343a4cac99470c03e23ba2bdc2bc', 48, 39, 37, 65, NULL, NULL, NULL),
(56, '7688b6ef52555962d008fff894223582c484517cea7da49ee67800adc7fc8866', 49, 39, 37, 65, NULL, NULL, NULL),
(57, 'c837649cce43f2729138e72cc315207057ac82599a59be72765a477f22d14a54', 50, 39, 37, 65, NULL, NULL, NULL),
(58, '6208ef0f7750c111548cf90b6ea1d0d0a66f6bff40dbef07cb45ec436263c7d6', 52, 39, 36, 65, NULL, NULL, NULL),
(60, '39fa9ec190eee7b6f4dff1100d6343e10918d044c75eac8f9e9a2596173f80c9', 42, 39, 37, 65, 'true', '2015-04-27 13:26:01', NULL),
(61, 'd029fa3a95e174a19934857f535eb9427d967218a36ea014b70ad704bc6c8d1c', 42, 39, 38, 62, 'true', '2015-04-28 22:07:27', NULL),
(62, '81b8a03f97e8787c53fe1a86bda042b6f0de9b0ec9c09357e107c99ba4d6948a', 42, 39, 39, 62, NULL, NULL, NULL),
(63, 'da4ea2a5506f2693eae190d9360a1f31793c98a1adade51d93533a6f520ace1c', 32, 39, 36, 66, NULL, NULL, NULL),
(204, 'fc56dbc6d4652b315b86b71c8d688c1ccdea9c5f1fd07763d2659fde2e2fc49a', 54, 39, 41, 69, NULL, NULL, 'tedsrate.rep/rater.php?&asid=fc56dbc6d4652b315b86b71c8d688c1ccdea9c5f1fd07763d2659fde2e2fc49a'),
(205, 'f8809aff4d69bece79dabe35be0c708b890d7eafb841f121330667b77d2e2590', 54, 39, 42, 69, NULL, NULL, 'tedsrate.rep/rater.php?&asid=f8809aff4d69bece79dabe35be0c708b890d7eafb841f121330667b77d2e2590'),
(206, '5cf4e26bd3d87da5e03f80a43a64f1220a1f4ba9e1d6348caea83c06353c3f39', 54, 39, 39, 69, NULL, NULL, 'tedsrate.rep/rater.php?&asid=5cf4e26bd3d87da5e03f80a43a64f1220a1f4ba9e1d6348caea83c06353c3f39');
...
CREATE TABLE IF NOT EXISTS `category` (
  ...
  `categoryLanguageID` int(11) NOT NULL,
...
INSERT INTO `category` (`categoryID`, `categoryName`, `criterionID`, `categoryLanguageID`, `categoryDesc`) VALUES
...
CREATE TABLE IF NOT EXISTS `cluster` (
  ...
  `clusterDesc` varchar(255) DEFAULT NULL
...
(89, 'this comment updated 4th', '2016-01-22 21:53:51', 54);
...
INSERT INTO `criterion` (`criterionID`, `criterionName`, `criterionDesc`, `criterionLanguageID`) VALUES
...
INSERT INTO `persona` (`personaID`, `personaName`, `personaDesc`, `personaLanguageID`) VALUES
...
INSERT INTO `project` (`projectID`, `projectName`, `projectDescription`, `projectLanguageID`) VALUES
...
INSERT INTO `scenario` (`scenarioID`, `scenarioName`, `scenarioDescription`, `scenarioLanguageID`) VALUES
...
INSERT INTO `scenarioCategory` (`scenarioCategoryID`, `scenarioID`, `categoryID`) VALUES
(901, 36, 7),
(902, 36, 8),
(903, 36, 9),
(904, 36, 10),
...
INSERT INTO `user` (`userID`, `email`, `firstName`, `lastName`, `preferredLanguage`, `passwordValue`, `AuthorityLevel`) VALUES
...
INSERT INTO `userPersonae` (`userPersonaeID`, `userID`, `personaID`) VALUES
(15, 27, 20),
(17, 27, 21),
(21, 29, 20),
(22, 29, 34),
...
ALTER TABLE `artifact`
  ...
  ADD KEY `FK_ArtifactLanguageID` (`artifactLanguageID`) USING BTREE;
...
ALTER TABLE `category`
  ...
  ADD KEY `FK_categoryLanguage` (`categoryLanguageID`),
...
ALTER TABLE `criterion`
  ...
  ADD KEY `criterionLanguageID` (`criterionLanguageID`);
...
ALTER TABLE `persona`
  ...
  ADD KEY `FK_personaLanguageID` (`personaLanguageID`);
...
ALTER TABLE `rating`
  ...
  ADD KEY `userRating_ibfk_2` (`categoryID`);
...
ALTER TABLE `scenarioCategory`
  ADD PRIMARY KEY (`scenarioCategoryID`),
  ADD KEY `FK_Scenario` (`scenarioID`),
...
--
ALTER TABLE `assessment`
  MODIFY `assessmentID` int(11) NOT NULL AUTO_INCREMENT,AUTO_INCREMENT=207;
--
...
--
ALTER TABLE `rating`
  MODIFY `ratingID` int(11) NOT NULL AUTO_INCREMENT,AUTO_INCREMENT=4421;
--
-- AUTO_INCREMENT for table `rating_comment`
--
ALTER TABLE `rating_comment`
  MODIFY `rating_commentID` int(11) NOT NULL AUTO_INCREMENT,AUTO_INCREMENT=90;
--
-- AUTO_INCREMENT for table `rating_screenshot`
--
ALTER TABLE `rating_screenshot`
  MODIFY `rating_screenshotID` int(11) NOT NULL AUTO_INCREMENT,AUTO_INCREMENT=9;
--
...
--
ALTER TABLE `scenarioCategory`
  MODIFY `scenarioCategoryID` int(11) NOT NULL AUTO_INCREMENT,AUTO_INCREMENT=1541;
--
...
--
ALTER TABLE `userPersonae`
  MODIFY `userPersonaeID` int(11) NOT NULL AUTO_INCREMENT,AUTO_INCREMENT=77;
--
...
ALTER TABLE `artifact`
  ...
  ADD CONSTRAINT `artifact_ibfk_6` FOREIGN KEY (`artifactLanguageID`) REFERENCES `language` (`languageID`);
...
--
ALTER TABLE `category`
  ADD CONSTRAINT `category_ibfk_1` FOREIGN KEY (`categoryLanguageID`) REFERENCES `language` (`languageID`),
  ADD CONSTRAINT `category_ibfk_2` FOREIGN KEY (`criterionID`) REFERENCES `criterion` (`criterionID`);

--
-- Constraints for table `cluster_category`
--
ALTER TABLE `cluster_category`
  ADD CONSTRAINT `cluster_category_ibfk_1` FOREIGN KEY (`categoryID`) REFERENCES `category` (`categoryID`),
  ADD CONSTRAINT `cluster_category_ibfk_2` FOREIGN KEY (`clusterID`) REFERENCES `cluster` (`clusterID`);
...
--
ALTER TABLE `criterion`
  ADD CONSTRAINT `criterion_ibfk_1` FOREIGN KEY (`criterionLanguageID`) REFERENCES `language` (`languageID`);

--
-- Constraints for table `persona`
--
ALTER TABLE `persona`
  ADD CONSTRAINT `persona_ibfk_1` FOREIGN KEY (`personaLanguageID`) REFERENCES `language` (`languageID`);
...
--
ALTER TABLE `project`
  ADD CONSTRAINT `project_ibfk_1` FOREIGN KEY (`projectLanguageID`) REFERENCES `language` (`languageID`);
...
ALTER TABLE `rating`
  ...
  ADD CONSTRAINT `rating_ibfk_2` FOREIGN KEY (`categoryID`) REFERENCES `category` (`categoryID`);
...
--
ALTER TABLE `scenario`
  ADD CONSTRAINT `scenario_ibfk_1` FOREIGN KEY (`scenarioLanguageID`) REFERENCES `language` (`languageID`);

--
-- Constraints for table `scenarioCategory`
--
ALTER TABLE `scenarioCategory`
  ADD CONSTRAINT `scenarioCategory_ibfk_1` FOREIGN KEY (`scenarioID`) REFERENCES `scenario` (`scenarioID`),
  ADD CONSTRAINT `scenarioCategory_ibfk_2` FOREIGN KEY (`categoryID`) REFERENCES `category` (`categoryID`),
  ADD CONSTRAINT `scenarioCategory_ibfk_3` FOREIGN KEY (`scenarioID`) REFERENCES `scenario` (`scenarioID`),
  ADD CONSTRAINT `scenarioCategory_ibfk_4` FOREIGN KEY (`categoryID`) REFERENCES `category` (`categoryID`);
...
--
ALTER TABLE `user`
  ADD CONSTRAINT `user_ibfk_1` FOREIGN KEY (`preferredLanguage`) REFERENCES `language` (`languageID`);

--
-- Constraints for table `userPersonae`
--
ALTER TABLE `userPersonae`
  ADD CONSTRAINT `userPersonae_ibfk_1` FOREIGN KEY (`userID`) REFERENCES `user` (`userID`),
  ADD CONSTRAINT `userPersonae_ibfk_2` FOREIGN KEY (`personaID`) REFERENCES `persona` (`personaID`);
